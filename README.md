...
- `patches/static-phrases.json` 中的 363 条完整固定短语，包括硬件加速、智能体权限、快照、代码导出与工作区说明；1.2.14 新增覆盖引导流程、服务商连接、钢笔工具、渐变面板、浏览器导入工具栏与评论等界面。
...
- 工具悬停提示中的**快捷键字母**（如创建矩形的 `R`）、模型与服务商名称、混合模式等专业名词；属性面板的宽高字母徽标（W/H）会翻译为"宽/高"。
- Windows 文件选择器等系统界面，以及服务端动态返回的消息。

## 使用前准备

1. 在 Pen 的 **Help → About Pen** 确认版本为 **1.2.10、1.2.13 或 1.2.14**。
...
当前脚本只允许对 1.2.10、1.2.13 和 1.2.14 写入。如果版本不同，脚本会停止，不会修改 Pen。
